...
for ($i = 1; $i -le $Runs; $i++) {
    ...
    & (Join-Path $root "run_test.ps1") -Clients $Clients -Ramp $Ramp -Warmup $Warmup -Duration $Duration `
...
$allPassed = (@($results | Where-Object { $_.ConnectionRate -lt 99 -or $_.PingRate -lt 99 -or $_.P95 -gt 100 -or $_.P99 -gt 200 -or $_.Errors -gt 0 }).Count -eq 0)
...
<header class="top"><div><div class="sub">REPEATED PERFORMANCE VERIFICATION</div><h1>Monster Breakers Server · $Runs-Run Benchmark</h1><div class="sub">동일 조건 반복 측정 · $Clients clients · warm-up $Warmup sec · measure $Duration sec · move $MoveHz/s · ping $PingHz/s</div></div><div class="badge $verdictClass">$verdict</div></header>
...
<section class="panel" style="margin-top:14px"><div class="label">원본 CSV SHA-256</div><div class="hash">$hashes</div></section><div class="foot">Generated $generated · 합격 기준: 접속률/Ping 응답률 99% 이상, P95 100ms 이하, P99 200ms 이하, 오류 0건</div>
</main></body></html>
"@
